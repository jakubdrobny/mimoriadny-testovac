...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if user:
            raise ValidationError('Toto používateľské meno je už zabrané. Prosím, zvoľ si iné.')

    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user:
            raise ValidationError('Na tento mail je už účet vytvorený.')

class LoginForm(FlaskForm):
    username = StringField('Používateľské meno', validators=[DataRequired(), Length(min=2, max=20)])
    password = PasswordField('Heslo', validators=[DataRequired()])
    remember = BooleanField('Zapamätaj si ma')
    submit = SubmitField('Prihlásiť sa')

class UpdateProfileForm(FlaskForm):
    username = StringField('Používateľské meno', validators=[DataRequired(), Length(min=2, max=20)])
    email = StringField('Email', validators=[DataRequired(), Email()])
    submit = SubmitField('Upraviť')
